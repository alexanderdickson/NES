 ...
 * together and advances them in lockstep (3 PPU dots + 1 APU cycle per CPU
 * cycle). Call {@link runFrame} once per displayed frame (~60 Hz).
 */
export class Nes {
  readonly ppu: Ppu;
  readonly apu: Apu;
  ...
  constructor(rom: NesRom, sampleRate = 44100) {
    const mapper = createMapper(rom);
    this.ppu = new Ppu(mapper);
    this.apu = new Apu(sampleRate);
    this.bus = new Bus(mapper, this.ppu, this.apu, this.controller1, this.controller2);
  ...
  private tick(): void {
    ...
    if (this.apu.irqPending()) this.cpu.requestIrq();
  }
